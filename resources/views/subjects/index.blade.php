    ...
    <div class="max-w-6xl mx-auto space-y-8 animate-fade-in">
        <div class="flex justify-between items-end border-b border-slate-200 pb-6">
            <div>
                ...
                <h1 class="font-classic text-4xl text-navy-900 transition-colors">Catálogo de Disciplinas
                </h1>
            </div>
            <button onclick="document.getElementById('modal-subject').classList.remove('hidden')"
                class="bg-navy-900 text-white px-6 py-3 rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-gold-600 hover:text-navy-950 transition-all shadow-lg shadow-navy-900/20">
                + Nova Disciplina
            </button>
        </div>
                ...
                <tbody class="divide-y divide-slate-50">
                    @foreach ($subjects as $subject)
                        <tr class="hover:bg-slate-50/50 transition-colors group">
                            <td
                                class="px-8 py-5 font-bold text-navy-900 uppercase tracking-tight group-hover:text-gold-500 transition-colors">
                                {{ $subject->name }}
                            </td>

                            <td class="px-8 py-5 text-center">
                                <span
                                    class="bg-navy-100 text-navy-600 text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wide">
                            ...
                            <td class="px-8 py-5 text-right">
                                <form action="{{ route('subjects.destroy', $subject) }}" method="POST"
                                    ...
                                    <button type="submit"
                                        class="text-red-400 hover:text-red-600 p-2 transition-colors rounded-xl hover:bg-red-500/5">
    ...
    <div id="modal-subject"
        ...
        <div class="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden animate-slide-up">
            ...
            <form action="{{ route('subjects.store') }}" method="POST" class="p-8 space-y-4">
                @csrf
                <div>
                    <label class="text-[10px] font-black uppercase text-slate-400 mb-2 block">Nome (Ex: Matemática)</label>
